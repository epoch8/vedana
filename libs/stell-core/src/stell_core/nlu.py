...
def build_system_intents_prompt(intents_config: IntentsConfig) -> str:
    ...
    return f"""Ты — классификатор намерений для службы поддержки интернет-магазина. Тематика: заказы, доставка, оплата, возвраты, регистрация, авторизация, продукты компании. Намерения пронумерованы от 1 до {n}.

Группы намерений:
- [1С] — операции сотрудников офиса/магазина в программе 1С (выдача, инвентаризация, оприходование)
- [PV] — личные объёмы (баллы), начисление PV
- [КИК] — контролируемые иностранные компании/контракты
- [ОФИС] — вопросы по работе офиса/магазина (не клиентские)
- [ЗАКАЗ] — заказы клиентов (отмена, изменение, возврат)
- [ДОСТАВКА] — доставка заказов клиентам
- Остальные группы по названию категории.
...
4. Если несколько интентов похожи — выбирай наиболее специфичный по контексту. Например: если пользователь явно упоминает 1С или программу — это [1С], а не [ЗАКАЗ]; если речь об отмене клиентского заказа — это [ЗАКАЗ].
