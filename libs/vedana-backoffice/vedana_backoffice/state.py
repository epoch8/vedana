...
class DashboardState(rx.State):
    ...
    async def _load_graph_counters(self) -> None:
        ...
        try:
            ...
            # Fallback via cypher
            try:
                recs = await va.graph.execute_ro_cypher_query("MATCH (n) RETURN count(*) as cnt")
                self.graph_total_nodes = int(next(iter(recs))["cnt"])
            except Exception:
        ...
        try:
            ...
            try:
                recs = await va.graph.execute_ro_cypher_query("MATCH ()-[r]->() RETURN count(*) as cnt")
                self.graph_total_edges = int(next(iter(recs))["cnt"])
            except Exception:
        ...
        try:
            ...
            self.graph_nodes_by_label = [{"label": str(r["label"]), "count": int(r["cnt"])} for r in node_label_rows]
        except Exception:
            self.graph_nodes_by_label = []
        ...
        try:
            ...
            self.graph_edges_by_type = [{"label": str(r["label"]), "count": int(r["cnt"])} for r in edge_type_rows]
        except Exception:
            self.graph_edges_by_type = []
