...
class LLM:
    ...
    async def extract_attributes_from_cypher(self, cypher_query: str) -> dict[str, Any]:
        ...
        messages: list[ChatCompletionMessageParam] = [
            ...
            {"role": "system", "content": base_helper_prompt_1},
    ...
    async def filter_graph_structure(self, graph_descr: str, natural_language_query: str) -> str:
        ...
        messages: list[ChatCompletionMessageParam] = [
            # TODO отрефакторить промпты в корректный формат - системный промпт-инструкция и юзерский - контент.
            {"role": "system", "content": base_helper_prompt_2},
            {"role": "user", "content": prompt},
        ]
...
base_helper_prompt_1 = "Ты помощник по работе с базами данных."
base_helper_prompt_2 = "Ты — помощник по работе с графовыми базами данных."
base_helper_prompt_3 = "Ты — помощник по работе с графовыми базами данных, в которых используется подмножество Cypher, совместимое с NetworkX."
